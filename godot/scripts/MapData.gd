...
# Two passes:
#   1. Direct field merge — duchy.name/color/lord, county.earl.
#   2. Aggregation — income/garrison/population on each county = SUM of
#      its baronies in DesignData. (Baronies are the source of truth now.)
# Safe to call repeatedly; never touches geometry fields.
func _merge_design_overlay() -> void:
	...
	# Pass 2 — aggregate barony economy onto each county. Gives downstream
	# callers (UI panels, GameState seed, end_turn) a normal-looking
	# `counties[cn].income` etc. without them having to know baronies exist.
	for cn in counties.keys():
		var ids: Array = []
...
# Per-barony economy. Read directly from DesignData's authoritative
# per-LAD dict; the value was either an explicit BARONY_OVERRIDES entry or
# a deterministic per-LAD slice generated by extract_design.py.
#
# Args:
#   _county_name (String): kept for back-compat; not used.
#   barony_id (String): LAD13CD of the target barony.
# Returns:
#   Dictionary: {income, garrison, population, (name?)}
func aggregate_barony(_county_name: String, barony_id: String) -> Dictionary:
	var dd: Node = get_node_or_null("/root/DesignData")
	if dd != null and dd.has_method("barony_economy"):
		return dd.barony_economy(barony_id)
